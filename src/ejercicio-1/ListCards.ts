...
export class ListCards {
  ...
  list(): void {
    this.Cards.collection.forEach((card) => {
        console.log(card.id, card.name, card.mana, chalk.hex(Correspondencies[card.color])(Colors[card.color]), TypeLines[card.typeLine], Oddities[card.oddity], card.rules, card.strength ?? "", card.endurance ?? "", card.loyalty ?? "", card.marketValue); // Cuidado con mostrar undefined
    })
  }
}